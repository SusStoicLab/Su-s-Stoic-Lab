...
description: Finds high-leverage intervention points in complex systems by mapping feedback loops, identifying system archetypes (fixes that fail, shifting the burden, tragedy of the commons, limits to growth), and ranking interventions by Meadows' leverage hierarchy. Use when problems involve interconnected components with feedback loops, delays, or emergent behavior; when past solutions failed or caused unintended consequences; when identifying where to push for maximum effect; or when user mentions systems thinking, leverage points, feedback loops, causal loop diagrams, stocks and flows, or complex systems. 中文触发词：杠杆点、系统杠杆、梅多斯杠杆层级、反馈回路、系统基模、治标不治本、系统干预、哪里下手最有效。
---
## 中文使用说明（搬运补充）

- **中文名**：系统思维与杠杆点（Systems Thinking & Leverage Points）
- **中文触发词**：杠杆点、系统杠杆、梅多斯杠杆层级、反馈回路、系统基模、治标不治本、系统干预、哪里下手最有效
- **来源备注**：本 skill 搬运自 GitHub 仓库 `lyndonkl/claude`（目录 `skills/systems-thinking-leverage`），原作者 lyndonkl。仅作个人学习使用，未改动原文核心内容，仅补充中文索引与触发词。随附 `resources/`（methodology.md、template.md）一并搬运。

### 章节中文索引
| 原文章节 | 中文速查 |
|------|------|
| Workflow（5 步清单） | ①定义系统与问题 ②画系统结构 ③找杠杆点 ④验证干预 ⑤设计高杠杆策略 |
| System Definition | 系统定义：边界 / 关键变量(存量·流量·目标) / 时间尺度 / 问题陈述(症状→模式→假设) |
| Leverage Points Analysis | 梅多斯 12 杠杆点（由弱到强）：参数<缓冲<结构<延迟<调节回路<增强回路<信息<规则<自组织<目标<范式<超越范式 |
| Validation | 验证清单：回路/存量流量/延迟/边界/杠杆分类/二阶效应/基模识别 |
| Delivery Format | 交付格式：系统概览 / 系统图 / 杠杆分析 / 干预策略 / 落地考量 |
| Common System Archetypes | 四大基模：治标失效、负担转移、公地悲剧、成长上限（含杠杆建议） |
| Quick Reference | 速查：关键概念(存量/流量/回路/延迟)、红旗预警 |
